...
export default async function CasamentoPage() {
  ...
  // 2. Buscando a Galeria de Fotos reais deste evento no Turso
  ...
  // Prevenção: Se o cliente ainda não mandou um vídeo, usamos um genérico de luxo
  const videoFundo = evento.video_url || "https://cdn.coverr.co/videos/coverr-wedding-reception-5241/1080p.mp4";

  return (
    <main className="min-h-screen bg-trevo-black text-trevo-white font-sans pb-24">
      {/* 1. BANNER DO EVENTO (AGORA COM VÍDEO DINÂMICO) */}
      <section className="relative h-[60vh] flex items-center justify-center text-center px-4 overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-5xl md:text-7xl font-serif text-trevo-gold mb-4 drop-shadow-lg">
            {evento.titulo as string}
          </h1>
          <p className="text-xl md:text-2xl font-light text-gray-300 max-w-2xl mx-auto drop-shadow-md">
            {evento.descricao as string}
          </p>
        </div>
      </section>

      {/* 2. GALERIA DE FOTOS (PUXANDO DO BANCO) */}
      <section className="max-w-7xl mx-auto px-6 mt-16">
        ...
        {/* Se tiver fotos no banco, mostra o grid. Se não, avisa o admin. */}
        {fotosGaleria.length > 0 ? (
          ...
          <p className="text-center text-gray-500 italic">Nenhuma foto adicionada à galeria ainda. Acesse o Painel Admin para enviar!</p>
        )}
      </section>

      {/* 3. PACOTES (JÁ ESTAVA FUNCIONANDO) */}
